...
<head>
  ...
  <script>
    ...
    function pageSizeCheck() {
    	...
    	location.href = "BoardList.bo?bName=${bName}&pageSize="+pageSize;
    }
...
<body>
	...
	<!-- Breadcrumb End -->
	<div class="container-fluid">
		<h2 class="section-title position-relative text-uppercase mx-xl-5 mb-4"><span class="bg-secondary pr-3">${bTextName} 게시판 조건별 검색 리스트</span></h2>
		<div class="row px-xl-5">
	    <!-- List Start -->
	    <div class="col-lg-12 col-md-12">
	      <div class="row pb-3">
					<div class="col-12 mb-5"><i class="fa-solid fa-magnifying-glass text-success"></i> <b class="text-success">${searchTitle}</b>(으)로 <b class="text-success">${searchString}</b>(을)를 검색한 결과 <b class="text-danger">${searchCount}</b> 건의 게시글이 검색되었습니다.</div>
	        <div class="col-12 pb-1">
	          <div class="d-flex align-items-center justify-content-between mb-4">
		          <div>
		            <c:if test="${sLevel < 4}"><a href="BoardInput.bo?bName=${bName}&pag=${pag}&pageSize=${pageSize}" class="btn btn-success btn-sm">글쓰기</a></c:if>
		          </div>
			        <div class="ml-2">
	      	...
	      	<c:set var="curScrStartNo" value="${curScrStartNo}" />
			    ...
			    <div class="col-lg-3 col-md-4 col-sm-6 pb-1">
	              ...
	              <div class="product-action">
	                <a class="btn btn-outline-dark btn-square" href="BoardContent.bo?bName=${bName}&idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}" title="게시글번호 : ${curScrStartNo}">${curScrStartNo}</a>
			  ...
			  <!-- 블록페이지 시작 -->
				<div class="text-center col-12">
				  <ul class="pagination justify-content-center">
					  <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=1&pageSize=${pageSize}">첫페이지</a></li></c:if>
					  <c:if test="${curBlock > 0}"><li class="page-item"><a class="page-link text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}">이전블록</a></li></c:if>
					  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize) + blockSize}" varStatus="st">
					    <c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link bg-primary border-secondary text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=${i}&pageSize=${pageSize}">${i}</a></li></c:if>
					    <c:if test="${i <= totPage && i != pag}"><li class="page-item"><a class="page-link text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=${i}&pageSize=${pageSize}">${i}</a></li></c:if>
					  </c:forEach>
					  <c:if test="${curBlock < lastBlock}"><li class="page-item"><a class="page-link text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=${(curBlock+1)*blockSize+1}&pageSize=${pageSize}">다음블록</a></li></c:if>
					  <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-dark" href="${ctp}/BoardList.bo?bName=${bName}&pag=${totPage}&pageSize=${pageSize}">마지막페이지</a></li></c:if>
				  </ul>
				</div>
				<!-- 블록페이지 끝 -->
	      ...
	      <!-- 검색기 시작 -->
				<div class="container mt-5">
				  <form name="searchForm" method="post" action="BoardSearchList.bo?bName=${bName}"  class="form-inline justify-content-center">
